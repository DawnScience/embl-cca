...
/*******************************************************************************
 * Copyright (c) 2010 Oak Ridge National Laboratory, EMBL.
 * All rights reserved. This program and the accompanying materials
 * are made available under the terms of the Eclipse Public License v1.0
 * which accompanies this distribution, and is available at
 * http://www.eclipse.org/legal/epl-v10.html
 ******************************************************************************/
//Originated from org.eclipse.nebula.visualization.xygraph.figures;
 ...
 *
 */
public class AnnotationEmbl extends Annotation implements IAxisListener, IDataProviderListener {
	/** The way how the cursor line will be drawn. */
	/**
	 * @author Xihui Chen
	 *
	 */
	public enum CursorLineStyleEmbl /*implements CursorLineStyle*/ {
		NONE(0, "None"),
		UP_DOWN(1, "Up&Down"),
		LEFT_RIGHT(2, "Left&Right"),
		FOUR_DIRECTIONS(3, "Four Directions"),
		NOCURSOR(4, "No Cursor");
		
		private CursorLineStyleEmbl(int index, String description) {
			 this.index = index;
		...
		public static String[] stringValues(){
			...
			int i=0;
			for(CursorLineStyleEmbl p : values())
				sv[i++] = p.toString();
			return sv;
		}	
		/**
	...
	private Axis yAxis;	
	private String name;
	private CursorLineStyleEmbl cursorLineStyle = CursorLineStyleEmbl.NONE;
	 ...
	 */	
	public AnnotationEmbl(String name, Trace trace) {
	 ...
	 * @param name the name of the annotation.
	 */
	public AnnotationEmbl(String name, Axis xAxis, Axis yAxis) {
		super(name, xAxis, yAxis); //No other solution, and this way using double resource
		//Undoing the forced construction, at least what can be done
		super.getXAxis().removeListener(this);
		super.getYAxis().removeListener(this);
		List children = super.getChildren();
		super.remove((IFigure)children.get(1));
		super.remove((IFigure)children.get(0));
	...
	protected void paintFigure(Graphics graphics) {
		if (isOpaque()) //Hack: Copied from super.super.paintFigure, which can not be called
		...
		case LEFT_RIGHT:
			...
			if(cursorLineStyle != CursorLineStyleEmbl.FOUR_DIRECTIONS)
				break;
		case UP_DOWN:
			//up
			graphics.drawLine(currentPosition.x , currentPosition.y - POINT_SIZE/2, 
	...
	protected void updateInfoLableText(boolean updateX0Y0) {
		if( infoLabel == null ) return; //Additional hack to avoid NPE when creating super
		String info = null;		
	 ...
	 */
	public void setCursorLineStyle(CursorLineStyleEmbl cursorLineStyle) {
...
/**
	...
	public CursorLineStyle getCursorLineStyle() {
		switch(cursorLineStyle) {
		case NONE:
			return CursorLineStyle.NONE;
		case UP_DOWN:
			return CursorLineStyle.UP_DOWN;
		case LEFT_RIGHT:
			return CursorLineStyle.LEFT_RIGHT;
		case FOUR_DIRECTIONS:
			return CursorLineStyle.FOUR_DIRECTIONS;
		default:
			return CursorLineStyle.NONE;
		}
	}
...
class InfoLabelDraggerEmbl extends MouseMotionListener.Stub implements MouseListener{
	...
	public void mousePressed(MouseEvent me) {
		command = new MovingAnnotationLabelCommand(AnnotationEmbl.this);
...
class PointerEmbl extends Figure {
	
	class PointerDraggerEmbl extends MouseMotionListener.Stub implements MouseListener{		
		...
		public void mousePressed(MouseEvent me) {
			command = new MovingAnnotationCommand(AnnotationEmbl.this);
			if(isFree())
	...
	public PointerEmbl() {
		...
		PointerDraggerEmbl dragger = new PointerDraggerEmbl();
